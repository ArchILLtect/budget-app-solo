...
function stripCardPrefixes(desc) {
    let s = desc;
...
function deriveVendorRoot(description, maxWords = 3) {
...
function recordVendorSample(ctx, vendorRoot, category, hadCategory) {
 ...
 */
export function inferCategoryPerTx(tx, ctx) {
...
export function applyConsensusCategories(
    ...
    for (let i = 0; i < accepted.length; i++) {
        if (accepted[i]._vendorRoot) {
            const { _vendorRoot, ...rest1 } = accepted[i];
            accepted[i] = rest1;
        }
    }
    return accepted;
