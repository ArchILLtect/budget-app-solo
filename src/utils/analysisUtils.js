...
export function findRecurringTransactions(transactions, options = {}) {
    const groups = {};
    ...
    for (const desc in groups) {
        let entries = groups[desc].sort((a, b) => a.date - b.date);
        ...
        const countMap = {};
        for (const amt of rounded) countMap[amt] = (countMap[amt] || 0) + 1;
        const mode = parseFloat(
            (Object.entries(countMap).sort((a, b) => b[1] - a[1])[0][0] / 100).toFixed(2)
        );
        ...
        const intervals = recent
            .slice(1)
            .map((entry, i) => (entry.date - recent[i].date) / (1000 * 60 * 60 * 24));

        const avgInterval = intervals.reduce((a, b) => a + b, 0) / intervals.length;
        const stdDev = Math.sqrt(
            intervals.map((i) => (i - avgInterval) ** 2).reduce((a, b) => a + b, 0) /
                intervals.length
        );
